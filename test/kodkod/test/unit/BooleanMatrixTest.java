...
public class BooleanMatrixTest {
	...
	/**
	 * @ensures sets the cells in m and the array, from range.min() to range.max(), to the corresponding values in vars.
	 */
	...
	/**
	 * @ensures sets the cells in m, from range.min() to range.max(), to the corresponding values in vars.
	 */
	...
	private void checkEquals(IntRange range, Iterator<IndexedEntry<BooleanValue>> indexIter) {
		for(int i = range.min(); i <= range.max(); i++) {
